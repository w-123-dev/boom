    ...
    for (var p = 0; p < this.players.length; p++) {
      ...
      if (player.x === cell2.x && player.y === cell2.y) {
        ...
        if (player.shielded) { player.shielded = false; }
          ...
          // Track kills: if bomber is not the dead player, bomber gets +1 kill
          if (bomb.ownerId !== player.id) {
            for (var ki = 0; ki < this.players.length; ki++) {
              if (this.players[ki].id === bomb.ownerId) {
                this.players[ki].kills++;
                break;
              }
            }
          } else {
